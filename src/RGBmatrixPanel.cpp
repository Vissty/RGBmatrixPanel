...
#define pinLO(x)	pinResetFast(x)//PIN_MAP[x].gpio_peripheral->BRR = PIN_MAP[x].gpio_pin
#define pinHI(x)	pinSetFast(x)//PIN_MAP[x].gpio_peripheral->BSRR = PIN_MAP[x].gpio_pin
...
IntoRobotIntervalTimer refreshTimer;
...
void RGBmatrixPanel::begin(void) {
    ...
    pinMode(_sclk , OUTPUT); pinResetFast(_sclk);//PIN_MAP[_sclk].gpio_peripheral->BRR = PIN_MAP[_sclk].gpio_pin;	//Low
    pinMode(_latch, OUTPUT); pinResetFast(_latch);//PIN_MAP[_latch].gpio_peripheral->BRR = PIN_MAP[_latch].gpio_pin;	//Low
    pinMode(_oe   , OUTPUT); pinSetFast(_oe);//PIN_MAP[_oe].gpio_peripheral->BSRR = PIN_MAP[_oe].gpio_pin;		//High  (disable output)
    pinMode(_a    , OUTPUT); pinResetFast(_a);//PIN_MAP[_a].gpio_peripheral->BRR = PIN_MAP[_a].gpio_pin;			//Low
    pinMode(_b    , OUTPUT); pinResetFast(_b);//PIN_MAP[_b].gpio_peripheral->BRR = PIN_MAP[_b].gpio_pin;			//Low
    pinMode(_c    , OUTPUT); pinResetFast(_c);//PIN_MAP[_c].gpio_peripheral->BRR = PIN_MAP[_c].gpio_pin;			//Low
    if(nRows > 8) {
        pinMode(_d  , OUTPUT); pinResetFast(_d);//PIN_MAP[_d].gpio_peripheral->BRR = PIN_MAP[_d].gpio_pin;			//Low
    }

    pinMode(R1, OUTPUT); pinResetFast(R1);//PIN_MAP[R1].gpio_peripheral->BRR = PIN_MAP[R1].gpio_pin;	//Low
    pinMode(G1, OUTPUT); pinResetFast(G1);//PIN_MAP[G1].gpio_peripheral->BRR = PIN_MAP[G1].gpio_pin;	//Low
    pinMode(B1, OUTPUT); pinResetFast(B1);//PIN_MAP[B1].gpio_peripheral->BRR = PIN_MAP[B1].gpio_pin;	//Low
    pinMode(R2, OUTPUT); pinResetFast(R2);//PIN_MAP[R2].gpio_peripheral->BRR = PIN_MAP[R2].gpio_pin;	//Low
    pinMode(G2, OUTPUT); pinResetFast(G2);//PIN_MAP[G2].gpio_peripheral->BRR = PIN_MAP[G2].gpio_pin;	//Low
    pinMode(B2, OUTPUT); pinResetFast(B2);//PIN_MAP[B2].gpio_peripheral->BRR = PIN_MAP[B2].gpio_pin;	//Low
...
// 4/4/4.  Then Adafruit_mfGFX (core library used across all Adafruit
...
// Promote 3/3/3 RGB to Adafruit_mfGFX 5/6/5
...
// Promote 4/4/4 RGB to Adafruit_mfGFX 5/6/5
...
// Demote 8/8/8 to Adafruit_mfGFX 5/6/5
...
uint16_t RGBmatrixPanel::Color888(
    ...
    if(gflag) { // Gamma-corrected color?
        r = pgm_read_byte(&gamma_1[r]); // Gamma correction table maps
        g = pgm_read_byte(&gamma_1[g]); // 8-bit input to 4-bit output
        b = pgm_read_byte(&gamma_1[b]);
...
uint16_t RGBmatrixPanel::ColorHSV(
    ...
    v1 = val + 1;
    if(gflag) { // Gamma-corrected color?
        r = pgm_read_byte(&gamma_1[(r * v1) >> 8]); // Gamma correction table maps
        g = pgm_read_byte(&gamma_1[(g * v1) >> 8]); // 8-bit input to 4-bit output
        b = pgm_read_byte(&gamma_1[(b * v1) >> 8]);
    } else { // linear (uncorrected) color
...
void RGBmatrixPanel::drawPixel(int16_t x, int16_t y, uint16_t c) {
    ...
    // Adafruit_mfGFX uses 16-bit color in 5/6/5 format, while matrix needs
...
{
    pinHI(signalPIN);	//Start timing
    //refreshTimer.interrupt_SIT(INT_DISABLE);
    activePanel->updateDisplay();   // Call refresh func for active display
    //refreshTimer.interrupt_SIT(INT_ENABLE);
}
...
void RGBmatrixPanel::updateDisplay(void) {
    ...
    if(++plane >= nPlanes) {      // Advance plane counter.  Maxed out?
        ...
        (row & 0x1) ? pinSetFast(_a):pinResetFast(_a);//PIN_MAP[_a].gpio_peripheral->BSRR = PIN_MAP[_a].gpio_pin : PIN_MAP[_a].gpio_peripheral->BRR = PIN_MAP[_a].gpio_pin;
        (row & 0x2) ? pinSetFast(_b):pinResetFast(_b);//PIN_MAP[_b].gpio_peripheral->BSRR = PIN_MAP[_b].gpio_pin : PIN_MAP[_b].gpio_peripheral->BRR = PIN_MAP[_b].gpio_pin;
        (row & 0x4) ? pinSetFast(_c):pinResetFast(_c);//PIN_MAP[_c].gpio_peripheral->BSRR = PIN_MAP[_c].gpio_pin : PIN_MAP[_c].gpio_peripheral->BRR = PIN_MAP[_c].gpio_pin;
        if(nRows > 8) {
            (row & 0x8) ? pinSetFast(_d):pinResetFast(_d);//PIN_MAP[_d].gpio_peripheral->BSRR = PIN_MAP[_d].gpio_pin : PIN_MAP[_d].gpio_peripheral->BRR = PIN_MAP[_d].gpio_pin;
        }
    }
    ...
    pinResetFast(_d);
        ...
        if(plane > 0) { // 188 ticks from TCNT1=0 (above) to end of function
            ...
            for (uint8_t i=0; i < 32; i++) {
                ...
                (ptr[i] & 0x04) ? pinSetFast(R1):pinResetFast(R1);//PIN_MAP[R1].gpio_peripheral->BSRR = PIN_MAP[R1].gpio_pin : PIN_MAP[R1].gpio_peripheral->BRR = PIN_MAP[R1].gpio_pin; //R1
                (ptr[i] & 0x08) ? pinSetFast(G1):pinResetFast(G1);//PIN_MAP[G1].gpio_peripheral->BSRR = PIN_MAP[G1].gpio_pin : PIN_MAP[G1].gpio_peripheral->BRR = PIN_MAP[G1].gpio_pin; //G1
                (ptr[i] & 0x10) ? pinSetFast(B1):pinResetFast(B1);//PIN_MAP[B1].gpio_peripheral->BSRR = PIN_MAP[B1].gpio_pin : PIN_MAP[B1].gpio_peripheral->BRR = PIN_MAP[B1].gpio_pin; //B1
                (ptr[i] & 0x20) ? pinSetFast(R2):pinResetFast(R2);//PIN_MAP[R2].gpio_peripheral->BSRR = PIN_MAP[R2].gpio_pin : PIN_MAP[R2].gpio_peripheral->BRR = PIN_MAP[R2].gpio_pin; //R2
                (ptr[i] & 0x40) ? pinSetFast(G2):pinResetFast(G2);//PIN_MAP[G2].gpio_peripheral->BSRR = PIN_MAP[G2].gpio_pin : PIN_MAP[G2].gpio_peripheral->BRR = PIN_MAP[G2].gpio_pin; //G2
                (ptr[i] & 0x80) ? pinSetFast(B2):pinResetFast(B2);//PIN_MAP[B2].gpio_peripheral->BSRR = PIN_MAP[B2].gpio_pin : PIN_MAP[B2].gpio_peripheral->BRR = PIN_MAP[B2].gpio_pin; //B2
            ...
            for(i=0; i<32; i++) {
                ...
                (bits & 0x04) ? pinSetFast(R1):pinResetFast(R1);//PIN_MAP[R1].gpio_peripheral->BSRR = PIN_MAP[R1].gpio_pin : PIN_MAP[R1].gpio_peripheral->BRR = PIN_MAP[R1].gpio_pin; //R1
                (bits & 0x08) ? pinSetFast(G1):pinResetFast(G1);//PIN_MAP[G1].gpio_peripheral->BSRR = PIN_MAP[G1].gpio_pin : PIN_MAP[G1].gpio_peripheral->BRR = PIN_MAP[G1].gpio_pin; //G1
                (bits & 0x10) ? pinSetFast(B1):pinResetFast(B1);//PIN_MAP[B1].gpio_peripheral->BSRR = PIN_MAP[B1].gpio_pin : PIN_MAP[B1].gpio_peripheral->BRR = PIN_MAP[B1].gpio_pin; //B1
                (bits & 0x20) ? pinSetFast(R2):pinResetFast(R2);//PIN_MAP[R2].gpio_peripheral->BSRR = PIN_MAP[R2].gpio_pin : PIN_MAP[R2].gpio_peripheral->BRR = PIN_MAP[R2].gpio_pin; //R2
                (bits & 0x40) ? pinSetFast(G2):pinResetFast(G2);//PIN_MAP[G2].gpio_peripheral->BSRR = PIN_MAP[G2].gpio_pin : PIN_MAP[G2].gpio_peripheral->BRR = PIN_MAP[G2].gpio_pin; //G2
                (bits & 0x80) ? pinSetFast(B2):pinResetFast(B2);//PIN_MAP[B2].gpio_peripheral->BSRR = PIN_MAP[B2].gpio_pin : PIN_MAP[B2].gpio_peripheral->BRR = PIN_MAP[B2].gpio_pin; //B2		PIN_MAP[_sclk].gpio_peripheral->BSRR = PIN_MAP[_sclk].gpio_pin;	//hi
                //PIN_MAP[_sclk].gpio_peripheral->BSRR = PIN_MAP[_sclk].gpio_pin;	//hi
                //PIN_MAP[_sclk].gpio_peripheral->BRR = PIN_MAP[_sclk].gpio_pin;	//lo
                pinSetFast(_sclk);
                    pinResetFast(_sclk);
